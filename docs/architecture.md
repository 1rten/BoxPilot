...
### 11.2 docker-compose（默认对外绑定，可通过 BIND_IP 收紧）
...
- API/UI 端口：按需暴露；公网部署建议配合防火墙/安全组限制来源 IP
- Proxy 端口：按需暴露；公网部署强烈建议限制来源 IP 并启用认证
- 不在仓库中提供任何节点/订阅示例（避免敏感）
- **绑定策略**：默认 compose 绑定 `0.0.0.0`；若仅本机访问，设置 `BIND_IP=127.0.0.1`。
